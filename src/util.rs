...
pub fn tilde_path(path: &PathBuf) -> PathBuf {
...
pub fn is_not_excluded(entry: &DirEntry, src_base: &PathBuf, excludes: &HashSet<PathBuf>) -> bool {
    let src = entry.path();
    let src_rel = src.strip_prefix(src_base).unwrap();
    if excludes.contains(src_rel) {
        return false;
    }
    return true;
}
